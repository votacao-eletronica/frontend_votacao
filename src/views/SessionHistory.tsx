...
export function SessionHistory() {
    ...
    return <main className="min-h-full bg-gray-50 p-8"><div className="mx-auto max-w-6xl">
        ...
        <header className="mb-8 rounded-lg bg-white p-6 shadow"><p className="text-sm font-bold uppercase text-blue-600">Histórico da sessão</p><h1 className="mt-2 text-3xl font-bold text-gray-900">{history.title}</h1><div className="mt-5 flex flex-wrap gap-5 text-sm text-gray-600"><span>Aberta: {history.opened_at ? new Date(history.opened_at).toLocaleString('pt-BR') : '-'}</span><span>Finalizada: {history.closed_at ? new Date(history.closed_at).toLocaleString('pt-BR') : '-'}</span><span className="flex items-center gap-1"><Users /> {history.attendees.length} presente(s)</span></div></header>
        ...
        <div className="space-y-6">{history.propositions.map((proposal, index) => <article key={proposal.id} className="rounded-lg bg-white p-6 shadow">
            <div className="flex justify-between gap-4"><div><p className="text-xs font-bold uppercase text-blue-600">Proposta {index + 1}</p><h2 className="text-xl font-semibold text-gray-900">{proposal.title}</h2></div><span className={`flex items-center gap-2 rounded-full px-4 py-2 font-bold ${proposal.result === 'approved' ? 'bg-green-100 text-green-700' : 'bg-red-100 text-red-700'}`}>{proposal.result === 'approved' ? <CheckCircle /> : <XCircle />}{proposal.result === 'approved' ? 'Aprovada' : 'Rejeitada'}</span></div>
